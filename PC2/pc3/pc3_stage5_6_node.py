...
class Stage56Node(Node):
    def __init__(self):
        ...
        self.sub_topic1 = self.create_subscription(
            Int32, '/signal_amr1_class', self.topic1_callback, self.qos, callback_group=self.group) # 변경
        self.sub_topic2 = self.create_subscription(
            Point, '/signal_amr1_map', self.topic2_callback, self.qos, callback_group=self.group) # 변경
        self.finish_pub = self.create_publisher(Int32, '/finish_signal_2', self.qos) # 추가
    ...
    def execute_sequence(self, obj_id):
        try:
            initial_pose = self.navigator.getPoseStamped([0.0463089, -6.241106], TurtleBot4Directions.SOUTH)
            initial_coord = [0.0463089, -6.241106] # 추가(도킹전 복귀좌표)
            ...
            first_goal = [0.0, -3.0] if obj_id in [1, 3] else [-1.8, -3.7]
